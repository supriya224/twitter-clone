...
import { HiSwitchHorizontal, HiOutlineChartBar } from "react-icons/hi";
...
function Post({ id, post, postPage }) {
  ...
  return (
    ...
    >
      {!postPage && (
        <img src={post?.userImg} className="w-11 h-11 rounded-full mr-4" alt="" />
      )}
      <div className="flex flex-col space-y-2 w-full">
        <div className={`flex ${!postPage && "justify-between"}`}>
          ...
          <div className="text-[#6e767d]">
            ...
            <span className=" pl-2 hover:underline text-sm sm:text-[15px]">
              <Moment fromNow>{post?.timestamp?.toDate()}</Moment>
            </span>
            {!postPage && (
              <p className="text-[#d9d9d9] text-[15px] mt-0.5">{post?.text} </p>
            )}
          </div>
        ...
        >
          ...
          >
            <div className="icon group-hover:bg-pink-600/10">
              {liked ? (
                <AiTwotoneHeart className="h-5 text-pink-600" />
              ) : (
          ...
          <div className="icon group">
            <PiShareFatBold className="h-5 group-hover:text-[#1d9bf0]" />
          </div>
          <div className="icon group">
